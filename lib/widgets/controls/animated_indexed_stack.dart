...
class AnimatedIndexedStack extends StatefulWidget {
  AnimatedIndexedStack(
      {super.key, required this.children, required this.index});
...
class _AnimatedIndexedStackState extends State<AnimatedIndexedStack>
  ...
  Widget build(BuildContext context) {
    return ScaleTransition(
      scale: Tween(
              begin: isAnimatingOut ? 1.0 : 0.9,
              end: isAnimatingOut ? 1.1 : 1.0)
          .animate(animation),
      child: FadeTransition(
        opacity: Tween(
                begin: isAnimatingOut ? 0.8 : 0.0,
                end: isAnimatingOut ? 0.0 : 1.0)
            .animate(animation),
        child: IndexedStack(
          index: _index,
